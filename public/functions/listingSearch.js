...
filterItems = function() {
    ...
    .then(data => {
        if(data.length == 0) alert("No items match your filters! Please try adjusting your spelling");
            ...
            data = data[0];
            var image = new Image();
            image.src = URL.createObjectURL(new Blob([data.image], {type: 'image/jpg'}));

            item =  '<div class="col-md-4 mb-4">';
            item += '<div class="card">';
            item += '<img class="card-img-top" src="' + image.src + '" alt="Listing Image"><div class="card-body">';
            item += '<h5 class="card-title">' + data.title + '</h5>';
            item += '<p class="card-text">' + data.description + '</p>';
            item += '<div class="mb-2"><span class="badge bg-info">' + data.category + '</span></div><div class="row">';
            ...
            item += '<div class="col-3" style="font-size: 10px;">' + data.dimensions + '</div>';
            item += '<div class="col-3" style="font-size: 10px;">' + data.weight + '</div>';
            item += '<div class="col-3" style="font-size: 10px;">Time Posted</div>';
            item += '<div class="col-3" style="font-size: 10px;">Location</div>';
